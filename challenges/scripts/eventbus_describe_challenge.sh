#!/usr/bin/env bash
# eventbus_describe_challenge.sh
#
# Round-245 paired-mutation deep-doc challenge for EventBus.
#
# Validates that:
#   1. The deep-doc ledger (docs/test-coverage.md) lists every exported
#      symbol from pkg/{event,bus,filter,middleware}.
...
#      against the real EventBus, byte-preserving non-ASCII metadata.
#   4. The README enumerates the four packages and the round-245 anti-bluff
#      guarantees section.
#
...
if [ "${MUTATE}" -eq 1 ]; then
    ...
    # Plant a rename: PublishAsync → PublishBogus (real symbol must remain
    # absent from ledger to flag the drift).
    sed -i 's/PublishAsync/PublishBogus_MUTATED/g' "${TMP_LEDGER}"
    LEDGER_WORK="${TMP_LEDGER}"
    echo "=== EventBus Describe Challenge (anti-bluff-mutate mode) ==="
else
    echo "=== EventBus Describe Challenge (clean mode) ==="
fi
echo ""
...
if [ ! -f "${LEDGER_WORK}" ]; then
    ...
    pass "ledger present"
    # Ensure round-245 marker present
    if grep -q "round-245" "${LEDGER_WORK}"; then
        pass "ledger marked round-245"
    else
        fail "ledger missing round-245 marker"
    fi
    # Ensure verbatim 2026-05-19 mandate quote present
...
# Extract exported top-level type/func names from each pkg.
# Format match: `func (...) Name(` or `func Name(` or `type Name `
# Hard-exclude *_test.go files at the file-glob level.
extract_symbols() {
    local pkg_dir="$1"
    local files
...
CHECKED=0
MISSING=0
for pkg in event bus filter middleware; do
    ...
    while IFS= read -r sym; do
        [ -z "${sym}" ] && continue
        CHECKED=$((CHECKED + 1))
        # Symbol may appear as `Sym`, `Sym(`, `Type.Sym`, `Sym(...)`. Match
        # any of those forms by searching for the bare word boundary too.
        if grep -qE "\b${sym}\b" "${LEDGER_WORK}"; then
            : # symbol cross-referenced
        else
            # Allow internal helpers we explicitly omitted; flag with FAIL
...
# Section 4: runner builds + runs against real EventBus
echo ""
...
if [ ! -f "${RUNNER}" ]; then
    ...
    cd "${MODULE_DIR}"
    if go build -o /tmp/eventbus_round245_runner ./challenges/runner/ 2>/tmp/eb_build.log; then
        pass "runner builds"
        if /tmp/eventbus_round245_runner -fixtures "${FIXTURE}" > /tmp/eb_run.log 2>&1; then
            pass "runner exit 0 against real EventBus"
            if grep -q "PASS \[sr\]" /tmp/eb_run.log; then
                pass "Cyrillic (sr) event round-trip"
            else
                fail "Cyrillic (sr) event missing from runner output"
            fi
            if grep -q "PASS \[ja\]" /tmp/eb_run.log; then
                pass "Japanese (ja) event round-trip"
            else
                fail "Japanese (ja) event missing from runner output"
            fi
        else
            fail "runner exit non-zero — see /tmp/eb_run.log"
            sed -n '1,20p' /tmp/eb_run.log
        fi
    else
        fail "runner build failed — see /tmp/eb_build.log"
        sed -n '1,20p' /tmp/eb_build.log
    fi
    rm -f /tmp/eventbus_round245_runner
fi

# Section 5: README round-245 anti-bluff section
echo ""
echo "Section 5: README round-245 anti-bluff section"
...
if grep -q "round-245" "${README}"; then
    pass "README marked round-245"
else
    fail "README missing round-245 marker"
fi
...
if [ "${MUTATE}" -eq 1 ]; then
    # In mutate mode, we EXPECT failures (ledger should miss PublishAsync).
    if [ "${FAIL}" -gt 0 ]; then
